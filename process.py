...
def find_in_journal(drug,Drugs,Pubmed,Clinals) :
    ...
    list = []
    for pub in Pubmed:
        if Drugs[drug].lower() in Pubmed[pub]['title'].lower() and {Pubmed[pub]['journal']: Pubmed[pub]['date']} not in list:
            list.append({Pubmed[pub]['journal']: Pubmed[pub]['date']})
    for pub in Clinals:
        if Drugs[drug].lower() in Clinals[pub]['title'].lower() and {Clinals[pub]['journal']: Clinals[pub]['date']} not in list:
